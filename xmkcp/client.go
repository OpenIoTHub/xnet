...
func Dial(raddr string) (*Client, error) {
	...
	serverNetIP, serverPort, err := xaddr.ParseHostAddrOnline(raddr)
	if err != nil {
		return nil, err
	}
	...
	/*fmt.Println(config.GetReadBufferSize(), config.GetMTUValue(), config.GetReceivingBufferSize())*/
	ctx := internet.ContextWithTransportSettings(context.Background(), &config)
	clientConn, err := DialKCP(ctx, v2net.UDPDestination(v2net.ParseAddress(serverIP), v2net.Port(serverPort)))
