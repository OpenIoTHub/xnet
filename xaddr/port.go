...
func IsLocalPortUsing(nettype string, port int) (bool, error) {
	if nettype == "tcp" {
		...
		return false, errors.New("unsupported net type " + nettype)
	}
}
...
func whoIsListeningLocalPort(nettype string, port int) (xproc.ProcId, error) {
	if runtime.GOOS == "windows" {
		return xproc.InvalidProcId, errors.New("Windows unsupported for now")
	} else {
		...
		if result == "" {
			return xproc.InvalidProcId, errors.New("port is not using")
		}
...
func CheckLocalPort(nettype string, port int) (isUsing bool, usingByWho xproc.ProcId, err error) {
	...
	if !IsValidPort(port) {
		return false, xproc.InvalidProcId, errors.New("invalid input port " + strconv.FormatInt(int64(port), 10))
	}
	if nettype != "tcp" && nettype != "udp" {
		return false, xproc.InvalidProcId, errors.New("unsupported network type " + nettype)
	}
...
func GetRandomAvailablePort(nettype string) (int, error) {
	...
	for {
		port := xrandom.RandomInt(1024, 65535)
