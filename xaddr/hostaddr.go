...
func ParseHostAddrOnline(addr string) (IP net.IP, port int, err error) {
	if len(addr) == 0 {
		return nil, -1, errors.New("Empty address string")
	}
	addr = strings.ToLower(addr)

	var addrWithoutPort string = addr
	...
	if e == nil && a.Port > 0 {
		...
		if end < 0 {
			return nil, -1, errors.New(addr + " is not a legal host address")
		}
	...
	if len(addrWithoutPort) == 0 {
		...
		if e != nil {
			return nil, -1, errors.New("Can't lookup IP from address string " + addr)
		}
		return ips[0], resultPort, nil
	} else {
		return nil, -1, errors.New(addr + " is not a legal socket address")
	}
}
