...
func msgToMail(msg *imap.Message, sec *imap.BodySectionName) (*Mail, error) {
	if msg == nil {
		return nil, errors.New("Empty source envelope")
	}
	...
	for {
		...
		if err == io.EOF {
			...
			return nil, errors.Wrap(err, "msgToMail -> NextPart")
		}
		...
		case mail.TextHeader:
			...
			if err != nil {
				return nil, errors.Wrap(err, "msgToMail -> ioutil.ReadAll")
			}
			c.BodyString = string(b)
			t, _, err := h.ContentType()
			if err != nil {
				return nil, errors.Wrap(err, "msgToMail -> ContentType")
			}
			if t == "text/plain" {
		...
		case mail.AttachmentHeader:
			...
			if err != nil {
				return nil, errors.Wrap(err, "msgToMail -> Filename")
			}
			b, _ := ioutil.ReadAll(p.Body)
			if err != nil {
				return nil, errors.Wrap(err, "msgToMail -> AttachmentHeader -> ioutil.ReadAll")
			}
			atch := Attachment{}
...
func decode(s string) string {
	...
	if xstring.StartWith(s, "=?GBK?B?") && xstring.EndWith(s, "?=") {
		s = xstring.RemoveHead(s, 8)
		s = xstring.RemoveTail(s, 2)
		gbk = true
	}
...
func (s *Receiver)  GetMails(box string, uid []int) ([]Mail, error) {
	if uid == nil || len(uid) == 0 {
		return nil, errors.New("Invalid input Uid")
	}
	...
	if len(recvMsgs) == 0 {
		return nil, errors.New("Read null mail")
	}
