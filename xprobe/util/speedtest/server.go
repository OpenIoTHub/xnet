...
func getAllTestServers(user *User) (*TestServerList, error) {
	...
	if resp.StatusCode != 200 {
		return nil, errors.New("Fail to get speedtest server list, status code " + strconv.FormatInt(int64(resp.StatusCode), 10))
	}
	if len(body) == 0 {
		return nil, errors.New("Fail to get speedtest server list, empty response!")
	}
	...
	if len(l.Servers) == 0 {
		return nil, errors.New("No speedtest server got")
	}
...
func (l *TestServerList) selectNearestTestServers(selectCount int) (*TestServerList, error) {
	if selectCount <= 0 {
		return nil, errors.New("Can't select " + strconv.FormatInt(int64(selectCount), 10) + " servers")
	}
	if len(l.Servers) == 0 {
		return nil, errors.New("Can't search from empty server list")
	}
	...
	selectCount = xnum.MinInt(len(l.Servers), selectCount)
	for i := 0; i < selectCount; i++ {
		result.Servers = append(result.Servers, l.Servers[i])
		xlog.Info("Test server: " + result.Servers[i].GetInfo())
	}
